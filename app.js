...
app.get('/', function (req, res) {
	if(has_conn)
	{
		...
		var js='<script>$(document).ready(function(){$("#submit").click(function(){var sea=$("#search").val();$.post("http://10.11.11.33:3000/search",{search: sea}, function(data){console.log(data);if(data=="done"){window.location="http://10.11.11.33:3000/listing"}});});})</script>\n';
		var incl='<script src="http://10.11.11.33:3000/readjquery"></script></html>\n';
		res.send(str+incl+js);
	}
	else
	{
		res.send('<html><p>Connecting to the hub</p><meta http-equiv="refresh" content="1; URL=http://10.11.11.33:3000"></html>');
	}
});
...
app.get('/download',function(req,res){
	res.send('<html><meta http-equiv="refresh" content="5; URL=http://10.11.11.33:3000/stream"></html>');
});
  ...
  fs.exists(path2, function(exists) {
    ...
    else
    {
         fs.exists(path1, function(exist) {
            ...
            {
                res.send('<html><p>Downloading!!!Please Wait!!!</p><meta http-equiv="refresh" content="1; URL=http://10.11.11.33:3000/stream"></html>');
            }
          });
    }
  });
...
app.get('/listing',function(req,res){
	var html='',js='<script src="http://10.11.11.33:3000/readjquery"></script>';
  		...
  		if (err) {
  		 ...
  		 for(var i=1;i<data.length;i+=2)
  		 {
  		 	...
  		 	if(re.test(data[i+1])|re1.test(data[i+1])|re2.test(data[i+1])|re3.test(data[i+1]))
  		 	{
				...
				html+='<td>'+data[i]+'</td>';
				html+='</tr>';
				js+='<script>$(document).ready(function(){$("#'+((i/2)|0)+'").click(function(){$.post("http://10.11.11.33:3000/download",{no:'+((i/2)|0)+'}, function(data){console.log(data);if(data=="done"){window.location="http://10.11.11.33:3000/download"}});});})</script>\n';
  		 	}
  		 	map[((i/2)|0)]=data[i+1];
  		 }
  		 //console.log(data);
  		res.send(html+js+'</table><meta http-equiv="refresh" content="5; URL=http://10.11.11.33:3000/listing"></html>');
		});
});
...
var js='<script src="http://10.11.11.33:3000/readjquery">  </script>';
var id=0;
fs.readdir('/home/arsenal/Downloads',function(err,files){
    ...
    files.forEach(function(file){
      ...
      js+='<script>$(document).ready(function(){$("#'+id+'").click(function(){window.location="http://10.11.11.33:3000/movie'+id+'";});})</script>';
...
fs.readdir('/home/arsenal/Downloads',function(err,files){
    ...
    files.forEach(function(file){
      ...
      {
        str+='<tr><td><button type="TEXT" id="'+id+'" size="40">'+file+'<br></td></td>';
      js+='<script>$(document).ready(function(){$("#'+id+'").click(function(){window.location="http://10.11.11.33:3000/partial'+id+'";});})</script>';
    map3[id]='/home/arsenal/Downloads/'+file;
    id+=1;
  }
  // do something with each file HERE!
    });
...
app.get('/partial*', function (req, res) {
  var id='';
  for(var i=8;i<req.url.length;i++)id+=req.url[i];
    console.log(id);
  var path = map3[id];
  console.log(path);
  var stat = fs.statSync(path);
